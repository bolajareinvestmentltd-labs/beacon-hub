...
export default async function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  try {
    relatedArticles = await getRelatedArticles(Number(article.id), 6);
  } catch (error) {
    console.warn("Failed to fetch related articles:", error);
  }

  // Calculate reading time from content
  const readingTime = getReadingTimeString(String(article.content || ""));
  ...
  return (
    <div className="w-full bg-gradient-to-b from-white via-slate-50 to-white dark:from-slate-950 dark:via-black dark:to-slate-950">
          ...
          <div>
            <p className="text-sm font-serif text-slate-600 dark:text-slate-400">
              ...
              <span className="font-bold text-black dark:text-white">
                {article.authorPerspective || article.author || "Editorial Board"}
              </span>
            </p>
      ...
      <AdPlacementManager>
        <EditorialColumnComponent content={article.content} accentColor="#E2725B" />
      </AdPlacementManager>
      ...
      {article.metaDescription && (
        <div className="max-w-4xl mx-auto px-4 md:px-8 lg:px-12 my-12 p-6 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10">
          <p className="text-sm text-slate-700 dark:text-slate-300 italic">{article.metaDescription}</p>
        </div>
      )}
