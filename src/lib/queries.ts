...
export async function getHeroArticle() {
  try {
    const hero = await runDbOperation(() =>
      db
        .select()
        .from(articles)
...
export async function getFeaturedArticles(limit = 4) {
  try {
    const featured = await runDbOperation(() =>
      db
        .select()
        .from(articles)
        .orderBy(articleOrder(articles.publishedAt))
...
export async function getBreakingNews(limit = 5) {
  try {
    const breaking = await runDbOperation(() =>
      db
        .select()
        .from(articles)
...
export async function getArticleMetrics(articleId: string) {
  try {
    const metrics = await runDbOperation(() =>
      db
        .select()
        .from(contentMetrics)
...
export async function getRelatedArticles(articleId: number, limit = 4) {
  try {
    const related = await runDbOperation(() =>
      db
        .select()
        .from(articles)
        .orderBy(articleOrder(articles.publishedAt))
...
export async function getEditorialSections() {
  try {
    const sections = await runDbOperation(() =>
      db
        .select()
...
export async function getHoroscopeBySign(sign: string | undefined | null) {
  try {
    ...
    const reading = await runDbOperation(() =>
      db
        .select()
        .from(horoscopes)
...
export async function getDeals(limit = 8) {
  try {
    const allDeals = await runDbOperation(() =>
      db
        .select()
        .from(deals)
